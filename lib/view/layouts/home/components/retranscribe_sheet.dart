...
/// The bulk re-transcription sheet, off the home menu. Club-gated whole, like
/// the entry export sheet: for a non-supporter the gate takes its place. The
/// run belongs to the root-scoped cubit, so dismissing the sheet stops
/// nothing; reopening lands on the live face.
...
class _IdleFace extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return SheetMessage(
      ...
      body: hasWork
          ? l10n.retranscribeIdleBody(state.runnable, state.current)
          : l10n.retranscribeAllCurrentBody,
      action: hasWork
