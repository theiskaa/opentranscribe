...
enum AppIconPickOutcome { switched, unchanged, failed }

@immutable
final class AppIconState {
  const AppIconState({required this.options, this.currentId, this.busy = false});
  ...
  AppIconState copyWith({String? currentId, bool? busy}) => AppIconState(
    options: options,
...
class AppIconCubit extends Cubit<AppIconState> {
  AppIconCubit({required this._store, required List<AppIconDescriptor> options})
    : super(AppIconState(options: options));
