...
void main() {
  group('viewedDayAt', () {
    ...
    final sortedStarts = [(203.0, jul23), (600.0, jul22), (1100.0, jul20)];
    const line = 190.0;
    ...
    test('home is today, and so is an overscroll past it', () {
      expect(viewedDayAt(0, line, sortedStarts, null), isNull);
      expect(viewedDayAt(-30, line, sortedStarts, jul23), isNull);
    });
    ...
    test('the home boundary is held from both sides', () {
      ...
      expect(viewedDayAt(12, line, sortedStarts, null), isNull);
      expect(viewedDayAt(13, line, sortedStarts, null), jul23);
      // Coming back, the day keeps it until the list is actually at rest, so
      // the same few pixels cannot hand it back.
      expect(viewedDayAt(1, line, sortedStarts, jul23), jul23);
      expect(viewedDayAt(0, line, sortedStarts, jul23), isNull);
    });
    ...
    test('a day takes the title the moment its label reaches the line', () {
      ...
      expect(viewedDayAt(12.9, line, sortedStarts, null), isNull);
      expect(viewedDayAt(13, line, sortedStarts, null), jul23);
      // jul22 starts at 600: it lands at 410.
      expect(viewedDayAt(409, line, sortedStarts, jul23), jul23);
      expect(viewedDayAt(410, line, sortedStarts, jul23), jul22);
    });
    ...
    test('a day parked on the line answers from geometry alone', () {
      ...
      expect(viewedDayAt(410, line, sortedStarts, null), jul22);
      expect(viewedDayAt(910, line, sortedStarts, null), jul20);
    });

    test('the holder keeps the title until its label falls a deadband back', () {
      expect(viewedDayAt(398, line, sortedStarts, jul22), jul22);
      expect(viewedDayAt(397, line, sortedStarts, jul22), jul23);
      // The hold is what the deadband buys: geometry alone gives the day above.
      expect(viewedDayAt(398, line, sortedStarts, null), jul23);
    });

    test('a wider deadband widens the hold window', () {
      expect(viewedDayAt(360, line, sortedStarts, jul22, deadband: 50), jul22);
      expect(viewedDayAt(359, line, sortedStarts, jul22, deadband: 50), jul23);
    });

    test('deep offsets stay on the last section', () {
      expect(viewedDayAt(5000, line, sortedStarts, null), jul20);
    });
  });
}
