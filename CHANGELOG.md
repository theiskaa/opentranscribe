...
- Whisper: whisper.cpp joins SpeechAnalyzer and Dictation on the transcription screen. Pick it, pick a model (Tiny, Base, Small, Medium, or Large Turbo, each with its size and a quality word), and one download serves the ninety-nine languages every model knows (Large Turbo adds Cantonese), including the ones the Apple engines have no model for. The live text stays off under Whisper; the transcript lands after you stop. A long recording runs ten minutes at a time, so an hours-long entry costs the phone no more memory than a short one. The model card shows the download as it runs, dims a model too large for the phone, and lets you remove one you no longer want; the Cache screen counts what the models hold. A model that downloads but will not open on the phone says so, and trying again fetches it afresh.
- The one connection: downloading a model is the only thing the app ever does over the network. It fetches a public file from one pinned host, resumes an interrupted download where it stopped, retries on its own when the connection breaks mid-way (leaving the app included; the screen says so while a download runs), as often as the link keeps delivering, verifies the file before it counts, and sends nothing. Everything else keeps working in airplane mode, and the one-rule test now holds the code to exactly that one file.
...
- Progress on a long pass: while Whisper transcribes, the entry's bar shows how far the run is, and home's bar shows it for a take being saved, the model download ahead of it included. A hot phone gets fewer whisper threads from its next run on.
- Ink sized to the take: under Whisper, which hears nothing live, a take waiting for its words holds their place with ink shaped like the words to come. On home it takes the shape of the row the record becomes, so a close guess lands without a jump; recording more lays the ink after your last word, where the new words will start. The size comes from how long you actually spoke (the silence before your first word, after your last, and in long pauses counts for nothing) and from how fast you speak each language, which the phone learns from your own takes and keeps to itself. Under the Apple engines the words heard live stand in instead, until the pass lands.
- Switching language mid-take: a take that changes language is cut at the pause around your pick, not the instant you tapped, so the words said just before or after the switch land whole and in their own language. Under Whisper, a sentence you said in the new language before reaching for the menu, or finished in the old one after, lands on its own side of the switch: Whisper listens for which of the two languages it is in. A stretch where you said nothing adds nothing (no stray "Thank you." from Whisper's silence), a marker appears only where the language actually changes, and a span that fails is tried once more; if it fails again the take is saved with its recording (and the words heard live, under an engine that shows them) instead of coming back translated into one language. Dictation, which cannot transcribe part of a recording, still hears the whole take in the language it opened in.
...
- The transcription screen: your language sits on top and stays put, and each engine's part slides in under the engine control. Tap an engine, drag the control, or swipe the page. While a recording runs the page won't switch, and an engine this iPhone can't run shows why.
- The model list works like the language list: the models you have, then the rest, each with what it's for. Tap one to use it or to download it; a download shows a ring you tap to stop it.
