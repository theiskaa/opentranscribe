...
- **Your data lives on your device.** Audio and transcripts are stored in the app's own on-device storage. Small settings are encrypted at rest with Fernet through `LocalService`. The storage encryption key is a build-time secret supplied with `--dart-define=STORAGE_KEY`; the default compiled into a plain debug build is a development placeholder, not a real key.
