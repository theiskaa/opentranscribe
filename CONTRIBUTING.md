...
**Nothing leaves the phone.** No network calls, no accounts, no analytics, no third-party SDK that phones home. The app has to keep working with the phone in airplane mode. The one carve-out is the model fetcher, which downloads a public Whisper model the user asked for from one pinned host and sends nothing; `test/one_rule_test.dart` holds the tree to that one file. Most of the architecture below follows from the rule, and a pull request that adds any other network call will be declined.
